...
import { 
  Search, Loader2, ChevronLeft, Table2, Calendar, Users, 
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col justify-between">
      ...
      <header className="border-b border-border/60 bg-card/80 backdrop-blur-md sticky top-0 z-50">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <Button size="sm" variant="outline" onClick={() => navigate('/leads')} className="hidden sm:flex border-border/80">
        ...
        <div className="max-w-xl mx-auto w-full pt-2 space-y-6">
            ...
            <CardContent className="p-6 sm:p-8 space-y-6">
              <form onSubmit={handleSearch} className="space-y-4">
                <div className="space-y-2">
                  <Label htmlFor="code" className="text-sm font-semibold flex items-center gap-1.5">
                    <Table2 className="h-4 w-4 text-muted-foreground" />
                    Reservation Code <span className="text-destructive">*</span>
                  </Label>
                  <div className="relative">
                    ...
                    <Input
                      ...
                      className="rounded-xl h-12 pl-11 text-base uppercase font-mono tracking-widest border-border/80 focus-visible:ring-primary"
                      disabled={loading}
                    />
                  </div>
                </div>
                ...
                <Button 
                  type="submit" 
                  disabled={loading} 
                  className="w-full rounded-xl font-bold bg-primary hover:bg-primary/95 text-primary-foreground h-12 text-base shadow-lg shadow-primary/20 active:scale-98 transition-all flex items-center justify-center gap-2"
                >
                  {loading ? (
          ...
          <AnimatePresence mode="wait">
            {reservation && (
              ...
              >
                {/* Premium Ticket Card */}
                <Card className="border-border/80 overflow-hidden shadow-xl rounded-3xl relative bg-card">
            ...
            {searched && !reservation && !loading && (
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="text-center p-8 bg-destructive/5 border border-destructive/10 rounded-3xl animate-fade-in"
              >
      ...
      <footer className="border-t border-border/60 py-6 text-center text-xs text-muted-foreground">
        <p>&copy; {new Date().getFullYear()} KaDel Ghana. Table Reservation Tracking.</p>
      </footer>
    </div>
  );
}
